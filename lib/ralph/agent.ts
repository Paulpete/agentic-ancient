...
export class RalphAgent {
  ...
  constructor() {
    ...
    // Strategy keys — must match exactly what database + belief-rewrite use
  ...
  async executeLoop() {
    ...
    for (const [name, strategy] of this.strategies.entries()) {
      try {
        ...
        if (!isEnabled && name !== 'belief') continue
        ...
        if (Math.random() < beliefScore || name === 'belief') {
          ...
          if (result.success && result.profitLoss !== undefined) {
            ...
            if (taxEvent) {
              console.log(
                `💰 [TAX] ${taxEvent.taxCategory} | $${taxEvent.gainLossUsd.toFixed(2)} | ${taxEvent.asset}`
              )
            }
          }
        ...
        // FIX: typed error handling — was `error.message` on untyped catch
        const message = err instanceof Error ? err.message : String(err)
        console.error(`❌ Strategy ${name} failed:`, message)
        await this.logExecution(name, { success: false, error: message })
      }
    }
  ...
  // FIX: return type was `boolean` — must be `Promise<boolean>` for async method
  private async isStrategyEnabled(name: string): Promise<boolean> {
    const strategy = await database.ralphStrategies.findUnique({ where: { name } })
